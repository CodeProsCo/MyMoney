                ...
                while (j < len) {
                    ...
                    if (rows[j]) {
                        ...
                        for (i = 0; i < l; i++) {
                            ...
                            c.empties[i] = (ts.getData(h, ch, "empty") ||
                                c.emptyTo ||
                                (c.emptyToBottom ? "bottom" : "top")).toLowerCase();
...
// faster than wrapInner
                        ...
                        this.order = formatSortingOrder(ts.getData($t, ch, "sortInitialOrder") || c.sortInitialOrder)
                            ? [1, 0, 2]
                            : [0, 1, 2];
            ...
            function setHeadersCss(table) {
                ...
                c.$headers.not(".sorter-false")
                    .each(function() {
                        var $this = $(this),
                            nextSort = this.order[(this.count + 1) % (c.sortReset ? 3 : 2)],
                            txt = $this.text() +
                                ": " +
                                ts.language[$this.hasClass(ts.css.sortAsc)
                                    ? "sortAsc"
                                    : $this.hasClass(ts.css.sortDesc) ? "sortDesc" : "sortNone"] +
                        ...
                        for (i = 0; i < l; i++) {
                            ...
                            if (num && c.strings[col]) {
                                ...
                                sort = c.numberSorter
                                    ...
                                    ](a[col], b[col], num, colMax[col], col, table);
                            } else {
            ...
            function bindMethods(table) {
                ...
                $table
                    ...
                    .bind("update" + c.namespace + " updateRows" + c.namespace,
                        function(e, resort, callback) {
                            e.stopPropagation();
                            table.isUpdating = true;
                        ...
                        function(e, cell, resort, callback) {
                            ...
                            if ($tb.length && tbdy >= 0) {
                                ...
                                if (typeof c.extractors[icell].id === "undefined") {
                                    ...
                                    t = c.extractors[icell]
                                        .format(getElementText(table, cell, icell), table, cell, icell);
                                }
                                v = c.parsers[icell].id === "no-parser"
                                    ? ""
                                    : c.parsers[icell].format(t, table, cell, icell);
                                c.cache[tbdy]
                                    .normalized[row][icell] =
                                    c.ignoreCase && typeof v === "string" ? v.toLowerCase() : v;
                                if ((c.parsers[icell].type || "").toLowerCase() === "numeric") {
                                    // update column max value (ignore sign)
                                    c.cache[tbdy].colMax[icell] = Math
                                        .max(Math.abs(v) || 0, c.cache[tbdy].colMax[icell] || 0);
                                }
                                checkResort($table, resort, callback);
                            }
                        })
                    .bind("addRows" + c.namespace,
                                ...
                                for (i = 0; i < rows; i++) {
                                    ...
                                    for (j = 0; j < l; j++) {
                                        if (typeof c.extractors[j].id === "undefined") {
                                            ...
                                            t = c.extractors[j]
                                                .format(getElementText(table, $row[i].cells[j], j),
                                                    table,
                                                    $row[i].cells[j],
                                                    j);
                                        }
                                        ...
                                        if ((c.parsers[j].type || "").toLowerCase() === "numeric") {
                                            // update column max value (ignore sign)
                                            c.cache[tbdy].colMax[j] = Math
                                                .max(Math.abs(cells[j]) || 0, c.cache[tbdy].colMax[j] || 0);
                                        }
                                    }
                    ...
                    .bind("updateComplete" + c.namespace,
                        function() {
                            table.isUpdating = false;
                        })
                    .bind("sorton" + c.namespace,
                    ...
                    .bind("applyWidgetId" + c.namespace,
                        function(e, id) {
                            e.stopPropagation();
                            ts.getWidgetById(id).format(table, c, c.widgetOptions);
                        })
                    .bind("applyWidgets" + c.namespace,
                        function(e, init) {
                            e.stopPropagation();
                            // apply widgets
                            ts.applyWidget(table, init);
                        })
                    .bind("refreshWidgets" + c.namespace,
                        function(e, all, dontapply) {
                            e.stopPropagation();
                            ts.refreshWidgets(table, all, dontapply);
                        })
                    .bind("destroy" + c.namespace,
                        function(e, c, cb) {
                            e.stopPropagation();
                            ts.destroy(table, c, cb);
                        })
                    .bind("resetToLoadState" + c.namespace,
                        function() {
                            // remove all widgets
                            ts.refreshWidgets(table, true, true);
            ...
            ts.setup = function(table, c) {
                ...
                if (!table || !table.tHead || table.tBodies.length === 0 || table.hasInitialized === true) {
                    return c.debug
                        ? log("ERROR: stopping initialization! No table, thead, tbody or tablesorter has already been initialized")
                        : "";
                }
                ...
                if (c.showProcessing) {
                    $table
                        ...
                        .bind("sortBegin" + c.namespace + " sortEnd" + c.namespace,
                            function(e) {
            ...
            ts.getColumnData = function(table, obj, indx, getCell) {
                ...
                if (obj[indx]) {
                    return getCell
                        ? obj[indx]
                        : obj[c.$headers.index(c.$headers.filter('[data-column="' + indx + '"]:last'))];
                }
                for (k in obj) {
                    if (typeof k === "string") {
                        if (getCell) {
            ...
            ts.bindEvents = function(table, $headers, core) {
                ...
                $headers
                    ...
                    .bind("mousedown mouseup sort keyup ".split(" ").join(c.namespace + " "),
                        function(e, external) {
                            var cell, type = e.type;
                            // only recognize left clicks or enter
                            if (((e.which || e.button) !== 1 && !/sort|keyup/.test(type)) ||
                            ...
                            cell = $.fn.closest
                                ...
                                : /TH|TD/.test(this.tagName) ? this : $(this).parents("th, td")[0];
            ...
            ts.destroy = function(table, removeClasses, callback) {
                ...
                c.$headers.add($f)
                    .removeClass([
                        ts.css.header, c.cssHeader, c.cssAsc, c.cssDesc, ts.css.sortAsc, ts.css.sortDesc,
                        ts.css.sortNone
                    ].join(" "))
...
// chunk/tokenize numbers & letters
    ...
    ts.addParser({
        ...
        format: function(s, table, cell, cellIndex) {
            if (s) {
                ...
                s = s
                    .replace(/\s+/g, " ")
                    .replace(/[\-.,]/g, "/"); // escaped - because JSHint in Firefox was showing it as an error
                if (format === "mmddyyyy") {
    ...
    ts.addParser({
        ...
        format: function(s, table) {
            return s
                ? ts.formatFloat((new Date("2000/01/01 " + s.replace(/(\S)([AP]M)$/i, "$1 $2")).getTime() || s), table)
                : s;
        },
        type: "numeric"
    });
